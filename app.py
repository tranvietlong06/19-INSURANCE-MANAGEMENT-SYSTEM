...
def highlight_expiring(row):
    """
    Styles the dataframe rows based on the ExpirationDate.
    Yellow for expiring within 30 days, Red for already expired.
    """
    from datetime import datetime, date, timedelta
    
    # Adjust 'ExpirationDate' to match the column name in your SQL View/DataFrame
    expiry = row['ExpirationDate']
    today = date.today()
    
    # If the expiry is a string, convert it to a date object
    if isinstance(expiry, str):
        expiry = datetime.strptime(expiry, "%Y-%m-%d").date()
        
    if expiry < today:
        return ['background-color: #ffcccc'] * len(row)  # Light Red for expired
    elif today <= expiry <= today + timedelta(days=30):
        return ['background-color: #fff9c4'] * len(row)  # Light Yellow for < 30 days
    else:
...
if choice == "Menu":
    ...
    st.title("🔍 Claims & Assessments")
    
    # Create tabs so you can switch between adding and viewing
    tab1, tab2 = st.tabs(["Log New Assessment", "Assessment History"])

    with tab1:
        st.subheader("Log Field Investigation")
        
        # Notice how EVERYTHING is indented under the form line now
        with st.form("new_assessment_form"):
            ...
            # The submit button MUST be indented inside the form
            submitted = st.form_submit_button("Log Assessment")
    ...
    with tab2:
        ...
        if rows:
            import pandas as pd # Just in case it's not at the top
            df_assess = pd.DataFrame(rows, columns=[
                "ID", "Contract ID", "Customer", "Date", "Status/Result"
            ])
            
            # 2. Add a filter so you can specifically check for "Denied"
            status_filter = st.selectbox("Filter by Status", ["All", "Approved", "Denied"])
            if status_filter != "All":
                df_assess = df_assess[df_assess['Status/Result'].str.contains(status_filter, case=False)]
